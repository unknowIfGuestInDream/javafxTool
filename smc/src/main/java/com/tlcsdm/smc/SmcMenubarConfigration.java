...
public class SmcMenubarConfigration implements MenubarConfigration {
    ...
    private final Action restart = new Action(I18nUtils.get("smc.menubar.file.restart"), actionEvent -> Platform.runLater(() -> {
        stage.close();
        try {
            Thread.sleep(1000);
    ...
    private final Action exit = new Action(I18nUtils.get("smc.menubar.file.exit"), actionEvent -> Platform.runLater(() -> {
        stage.close();
    ...
    private final Action contactSupport = new Action(I18nUtils.get("smc.menubar.help.contactSupport"), actionEvent -> openWeb(SmcConstant.GITHUB_PROJECT_SUPPORT_URL));

    private final Action submitFeedback = new Action(I18nUtils.get("smc.menubar.help.submitFeedback"), actionEvent -> openWeb(SmcConstant.GITHUB_PROJECT_FEEDBACK_URL));
    ...
    private final Action release = new Action(I18nUtils.get("smc.menubar.help.release"), actionEvent -> openWeb(SmcConstant.PROJECT_RELEASE_URL));

    private final Collection<? extends Action> actions = List.of(
            new ActionGroup(I18nUtils.get("smc.menubar.file"), getImageView("/com/tlcsdm/smc/static/menubar/file.png"), restart, exit),
            new ActionGroup(I18nUtils.get("smc.menubar.setting"), getImageView("/com/tlcsdm/smc/static/menubar/setting.png"),
                    new ActionGroup(I18nUtils.get("smc.menubar.setting.language"), getImageView("/com/tlcsdm/smc/static/menubar/language.png"), new CheckLangAction("中文简体"),
                            new CheckLangAction("English"), new CheckLangAction("日本語"))),
            new ActionGroup(I18nUtils.get("smc.menubar.help"), getImageView("/com/tlcsdm/smc/static/menubar/help.png"), contactSupport, submitFeedback, ACTION_SEPARATOR, release,
                    about));
    ...
    private ImageView getImageView(String url) {
        return new ImageView(new Image(Objects
                .requireNonNull(getClass().getResource(url)).toExternalForm()));
    }
