...
public class SmcMenubarConfigration implements MenubarConfigration {
    ...
    private final Action openSysConfig = FxAction.openSysConfig(actionEvent -> JavaFxSystemUtil.openDirectory(ConfigureUtil.getConfigurePath(Config.CONFIG_FILE_NAME)));

    private final Action openUserData = FxAction.openUserData(actionEvent -> JavaFxSystemUtil.openDirectory(ConfigureUtil.getConfigurePath(Config.USERDATA_FILE_NAME)));
    ...
    private final Collection<? extends Action> actions = List.of(
            FxActionGroup.file(restart, exit),
