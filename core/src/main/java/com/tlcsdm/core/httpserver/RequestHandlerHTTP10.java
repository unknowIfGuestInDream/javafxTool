...
abstract class RequestHandlerHTTP10 extends RequestHandler {
    ...
    protected void handle(Socket socket) throws IOException, SimpleWebServerException {
        ...
        // TODO Profile/research: Is DateFormat initialization an expensive operation?
        // Do this only once...
        ...
        // This is Connection: close is probably not strictly neccessary as we are a
        // HTTP/1.0 server so far here, but it can't work and seems to work well
        ...
        if (HTTPRequest.Version.HTTP11.toString().equals(request.getHTTPVersion())) {
            // Until HTTP/1.1 is properly implemented here, simply "force" (?) response to
            // 1.0 (http://www.ietf.org/rfc/rfc2145.txt)
            // I'm not 1000% sure if this is correct... but it seems to work well with
            // HTTP/1.1 browsers...
            ...
            throw new SimpleWebServerException(
                    "Don't know how to answer HTTP requests with this version header: " + request.getHTTPVersion());
        }

        this.handle(request, response);

        System.out.println(
                socket.getInetAddress().getHostAddress() + " [" + new Date().toString() + "] " + request.getMethod()
                        + " " + request.getHTTPVersion() + " " + request.getURI() + " " + response.getStatusCode());

        // TODO HTTP/1.1 support, we probably don't want to close this response
        // (ultimately, underlying socket) just yet and wait for more requests in this
        // same Handler?
        response.close();
    }
    ...
    private HTTPRequest getHTTPRequest(Socket socket) throws IOException, SimpleWebServerException {
        ...
        // TODO Security: Use e.g. a custom BufferedReader subclass that limits
        // characters per line and total lines to avoid DOS/exhaustion attacks.. (but
        // take big file uploads via POST into account!)
        httpRequestLine = bufferedReader.readLine();
        // This could throw a SocketTimeoutException, which will propagate to the
        // caller, as it should.
        // If null, this also indicates a timeout occured, and we are not dealing with
        // the request either...
        ...
        try {
            ...
            throw new SimpleWebServerException(
                    "HTTP Request Line (1st line) invalid, should be 'VERB URI VERSION' and not '" + httpRequestLine
