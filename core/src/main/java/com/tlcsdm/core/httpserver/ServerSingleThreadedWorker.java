...
class ServerSingleThreadedWorker extends Thread implements Runnable {
     ...
     */
    // TODO Alternative constructors, particularly with InetAddress bindingAddress
    // (maybe backlog?)
    ...
    public void run() {
        try {
            while (!isInterrupted()) {
                ...
                // Set timeout so that clients that are much too slow and would block the server
                // for other clients can get their butt kicked
                // NOTE: This is a setSoTimeout() on the SOCKET, not on the SocketServer (@see
                // unblockWaitingSocket)
                socket.setSoTimeout(timeout);

                RequestHandler handler = requestHandlerFactory.newRequestHandler(socket);
                this.handle(handler);
            }
        } // Note: An IOException from ServerSocket.accept() is NOT a recoverable
          // condition (i.e. just log it and keep waiting for the next won't help)
          // Its occurence indicates e.g. a configuration problem that would justify
          // aborting completly, i.e. stopping the server.
          // This is different from an Exception (IOException or other) occuring during
          // the handling of one specific request above.
        catch (IOException ex) {

            // This throw is probably not going anywhere anyway - there can be no catch for
            // this...
    ...
    public void terminate() {
        try {
            ...
            // close() failed? Boh, who cares, probably safe to ignore - caller could take
            // any sensible action; we simply want it down anyway.
        }
    }
